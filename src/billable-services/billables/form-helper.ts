...
import { type ExcelFileRow, type PaymentMethod, type ServiceType } from '../../types';
import { type ChargeAble } from './charge-summary.resource';
import { type BillableFormSchema, type ServicePriceSchema } from './form-schemas';
...
export const formatBillableServicePayloadForSubmission = (
  ...
  const formPayload = {
    ...
    servicePrices: formData.servicePrices.map((servicePrice) => ({
      ...(servicePrice.uuid ? { uuid: servicePrice.uuid } : {}),
...
export const getBulkUploadPayloadFromExcelFile = (
  ...
  const jsonData: Array<ExcelFileRow> = [];
